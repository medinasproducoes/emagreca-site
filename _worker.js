...
async function criarPagamento(request) {
  ...
  try {
    ...
    const orderNsu = `emagreca-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;

    const payload = {
      handle: 'medina_pay',
      items: [{ quantity: 1, price: 4290, description: 'Emagreça Sem Passar Fome - Método 2026' }],
      order_nsu: orderNsu,
...
async function webhookInfinitePay(request) {
  try {
    ...
    console.log('Venda confirmada (Emagreça Sem Passar Fome):', data.order_nsu, data.paid_amount);
